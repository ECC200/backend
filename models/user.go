...
type User struct {
	...
	// Date              []string           `json:"date" firestore:"date"`                               //日付の管理をするために配列にする
	PrimaryCareDoctor string `json:"primary_care_doctor" firestore:"primary_care_doctor"` //かかりつけ医
	MedicationStatus  string `json:"medicationstatu" firestore:"medicationstatus"`        //服薬中の薬
	DoctorComment     string `json:"doctorcomment" firestore:"doctorcomment"`             //主治医のコメント
}
